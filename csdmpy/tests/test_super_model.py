...
from csdmpy.super_model import (
    get_daily_entrants,
    get_daily_transition_rates,
    step_to_days,
    transition_probs_per_bracket,
    daily_entrants_per_bracket,
    ageing_probs_per_bracket,
    make_populations_ts,
    calculate_timestep_transition_matrices,
)
from csdmpy.utils import make_date_index
...
def test_get_daily_transition_rates(dummy_entrant_eps):
    ...
    assert (
        transition_rates.loc["Foster", "Foster"] == transition_rates.loc["Resi", "Resi"]
    )
    # check values
...
def test_transition_probs_per_bracket(dummy_entrant_eps, dummy_age_brackets):
    tran_probs = transition_probs_per_bracket(
        df=dummy_entrant_eps,
        bin_defs=dummy_age_brackets,
        start_date="18/05/2000",
        end_date="22/05/2000",
    )
...
def test_daily_entrants_per_bracket(dummy_entrant_eps, dummy_age_brackets):
    ent_probs = daily_entrants_per_bracket(
        df=dummy_entrant_eps,
        bin_defs=dummy_age_brackets,
        start_date="18/05/2000",
        end_date="22/05/2000",
...
def test_calculate_timestep_transition_matrices(dummy_entrant_eps, dummy_age_brackets):
    ts_info = make_date_index(
        start_date="22/05/2000", end_date="01/08/2003", step_size="5w"
    )
    daily_t_probs = transition_probs_per_bracket(
        df=dummy_entrant_eps,
        bin_defs=dummy_age_brackets,
        start_date="18/05/2000",
        end_date="22/05/2000",
    ...
    assert result[35]["16 to 18"].loc["Supported"].round(decimals=4).tolist() == [
        0.0119,
        0.0,
        0.0017,
    ]
